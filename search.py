...
def uniformCostSearch(problem):
    ...
    funcion_costo=lambda x:1 #lambda x: util.manhattanDistance(x,problem.goal)
    initial_state = problem.getStartState()
    initial_actions = []
    initial_candidate = (initial_state, initial_actions)
    priorityq.push(initial_candidate,funcion_costo(initial_candidate))
    closed_set = set()
    while not priorityq.isEmpty():
        ...
        if state not in closed_set:
            ...
            candidate_successors = map(lambda x: (x[0], actions + [x[1]]), candidate_successors)
            for candidate in candidate_successors:
                priorityq.push(candidate,funcion_costo(candidate))
